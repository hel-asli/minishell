...
builts = {
	...
	"pwd 1":r"pwd",
	"pwd 2":r"pwd oi",
	"pwd 3":r"pwd 12313123 1231  23 1",
	"pwd 4":r"pwd $sdfdsf '\"' 12313123 1231  23 1",
...
wildcard = {
	...
	"wildcard 13": '$USER*',
	"wildcard 14": '$lllll*****',
	"wildcard 15": 'echo $.sdfsdfsf',
	"wildcard 16": r"echo $\"\"$",
	"wildcard 17": r'echo $\"$\"$',
	"wildcard 18": r'echo $sdfsdf=*$dl=',
...
mix = {
	...
	"mix test 58":r'cd /tmp && touch file1 file2 && (export) > file* && rm -fr file1 file2',
	"mix test 59":r'cd /tmp && export a=\"hello world\" && (echo hello) >> $a',
	"mix test 60":r'cd /tmp && export a=\"hello world\" && (export) >> $a',
	"mix test 61":r'cd /tmp && export a=\"hello world\" && (echo hello) > $a',
	"mix test 62":r'cd /tmp && export a=\"hello world\" && (export) > $a',
	"mix test 63":r'cd /tmp && export a=\"hello world\" && (export && ls) > $a',
	"mix test 64":r'export +++ && echo $?',
	"mix test 65":r'cd /exam && echo $?',
	"mix test 66":r'cd /aaaaaaaa && echo $?',
	"mix test 67":r'()',
	"mix test 68":r' << ()',
...
def run_tests(tests):
	...
	for key in tests:
		...
		print("command :", f"\033[1;34m{command}\033[0m")
...
def get_minishell_return(command):
	...
	#print("outpuuuuut  : ",remove_fl_lines(got.stdout,command))
	return remove_fl_lines(got.stdout,command), got.stderr, got.returncode

def remove_fl_lines(output,command):
	#output = output.replace(f"minishell> {command}\n".replace('\\',''),"")
	output = output[output.find('\n') + 1:]
	output = output.replace("minishell> exit\n","")
	return output
